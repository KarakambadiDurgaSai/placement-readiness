...
const adminJsxPath = 'src/pages/admin/AdminDashboard.jsx';
let originalContent = fs.readFileSync(adminJsxPath, 'utf8');

const splitMarker = '  /* ── Loading ─────────────────────────────────────────────────── */';
const splitIndex = originalContent.indexOf(splitMarker);
...
const logicCode = originalContent.substring(0, splitIndex);

const newRenderCode = `  /* ── Loading ─────────────────────────────────────────────────── */
  if (loading) {
    return (
...
console.log('Successfully replaced AdminDashboard.jsx!');
